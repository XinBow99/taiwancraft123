...
上車時視角會自動轉到車頭方向，之後車轉多少、畫面就跟著轉多少（還是可以左右各看 105 度）。

轉向是照兩輪車來算的：方向鍵打的是**把手**，車會轉是因為「正在前進」而且「前輪有角度」：

    Δ車身朝向 = (速度 ÷ 軸距) × tan(把手角度)

所以停著不動時把手轉了車不會轉（速度是 0，乘出來就是 0），要動起來才轉得動。
把手能打的角度隨速度遞減——高速全打死等於摔車：
...
全速的迴轉半徑是 10.7 格，路口要先減速才過得去。放開方向鍵把手自己回正，車就直行。

甩尾則是刻意讓輪胎抓不住：車頭轉了、車身跟不上，那個差額就是甩出去的角度。
